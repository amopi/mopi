...
class InitializeProjectCommand extends Command
{
    ...
    protected function configure()
    {
        ...
        $this->setName('mopi:project:init');
...
class {$this->mainClassname} extends Mopi
{
}
 ...
 * Time: $time
 */
use Amopi\\Mlib\\ODM\\Dynamodb\\Console\\ConsoleHelper;
use {$this->projectNamespace}Database\\{$this->mainClassname}Database;
...
SRC;
        $odmImports                        = <<<SRC
use Amopi\Mlib\ODM\Dynamodb\ItemManager;
...
SRC;
    ...
    protected function prepareDatabaseRelatedFiles()
    {
        ...
        $question = new Question(
            "Do you want to enable ODM support (Amopi/DynamoDb-ODM)?"
            . " <info>[yes]</info>: ",
            'yes'
        );
        $answer   = $helper->ask($this->input, $this->output, $question);
...
SRC;
    ...
    protected function prepareServicesYaml()
    {
        ...
        if ($this->odmSupportEnabled) {
            $services['services']['item_manager']                               = [
                "class"   => "Amopi\\Mlib\\ODM\\Dynamodb\\ItemManager",
                "factory" => [
                    $this->projectNamespace . "Database\\" . $this->mainClassname . "Database",
                    "getItemManager",
                ],
    ...
    protected function updateComposerInfo()
    {
        ...
        if ($this->ormSupportEnabled) {
            system("composer require amopi/doctrine-addon", $retval);
            if ($retval == 0) {
        ...
        if ($this->odmSupportEnabled) {
            system("composer require amopi/dynamodb-odm", $retval);
            if ($retval == 0) {
